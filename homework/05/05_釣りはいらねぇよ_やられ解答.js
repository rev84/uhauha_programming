...
canPayJust = function(payYen, coinAry) {
  var coinMoneyAry = [1,5,10,50,100,500];//小銭の金額

  for(var index = coinAry.length - 1;index >= 0　&& payYen != 0;index--){
    var usemax = Math.floor(payYen / coinMoneyAry[index]);//最大で使える枚数
    if(coinAry[index] <= usemax){
      payYen = payYen - (coinAry[index] * coinMoneyAry[index]);
    }
  }
  if(payYen == 0){
    return true;
